...
static void IocsInitializeKeycode(void);
static void IocsClearKeycode(void);
static void IocsSetKeycode(int keycode);
static void IocsUpdateKeycode(void);
static void IocsInitializeAudio(void);
static void IocsFadeAudioMusicCallback(SoundSource *c);
static void IocsInitializeRandom(void);
...
void IocsInitialize(PlaydateAPI *playdate)
{
    ...
    // キーコードの初期化
    IocsInitializeKeycode();

    // オーディオの初期化
    IocsInitializeAudio();

    // 乱数の初期化
    IocsInitializeRandom();
...
static void IocsClearKeycode(void)
{
    ...
    // キーコードのクリア
    for (int i = 0; i < kIocsKeycodeSize; i++) {
        iocs->keycodePresseds[i] = false;
    }
}
...
static void IocsSetKeycode(int keycode)
{
    ...
    // キーコードの設定
    if (keycode < kIocsKeycodeSize) {
        iocs->keycodePresseds[keycode] = true;
    }
}

// キーコードが押されているかどうかを判定する
//
bool IocsIsKeycodePressed(int keycode)
{
    return keycode < kIocsKeycodeSize ? iocs->keycodePresseds[keycode] : false;
}
...
// ミュージックオーディオを再生する
//
void IocsPlayAudioMusic(const char *path, int repeat)
{
    // Playdate の取得
    PlaydateAPI *playdate = IocsGetPlaydate();
    if (playdate == NULL) {
    ...
    if (iocs->audioMusicPlayer != NULL) {
        ...
        playdate->sound->fileplayer->freePlayer(iocs->audioMusicPlayer);
        iocs->audioMusicPlayer = NULL;
    }
...
// ミュージックオーディオを停止する
//
void IocsStopAudioMusic(float fade)
{
    // Playdate の取得
    PlaydateAPI *playdate = IocsGetPlaydate();
    if (playdate == NULL) {
    ...
    if (fade == 0) {
        ...
        int len = (int)(fade * (float)kIocsAudioMusicSampleRate);
        playdate->sound->fileplayer->fadeVolume(iocs->audioMusicPlayer, 0.0f, 0.0f, len, IocsFadeAudioMusicCallback);
    }
}
static void IocsFadeAudioMusicCallback(SoundSource *c)
{
...
static void IocsInitializeRandom(void)
{
    iocs->xorshift.seed = 2463534242;
}

// 乱数の種を初期化する
//
void IocsSetRandomSeed(struct XorShift *xorshift, uint32_t seed)
{
    if (xorshift == NULL) {
        xorshift = &iocs->xorshift;
    }
    xorshift->seed = seed;
}
...
int IocsGetRandomRange(struct XorShift *xorshift, int minimum, int maximum)
{
    return (IocsGetRandomNumber(xorshift) % (maximum - minimum + 1)) + minimum;
}

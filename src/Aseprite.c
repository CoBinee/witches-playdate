...
void AsepriteDrawSpriteAnimation(struct AsepriteSpriteAnimation *animation, int x, int y, LCDBitmapDrawMode mode, LCDBitmapFlip flip)
{
    ...
    // ビットマップの描画
    {
        ...
        playdate->graphics->drawBitmap(bitmap->bitmap, x + frame->spriteSourceSize.x, y + frame->spriteSourceSize.y, flip);
    }
}
void AsepriteDrawRotatedSpriteAnimation(struct AsepriteSpriteAnimation *animation, int x, int y, float degrees, float centerx, float centery, float xscale, float yscale, LCDBitmapDrawMode mode)
{
    ...
    // ビットマップの描画
    {
        ...
        {
            float fx = (((float)-frame->sourceSize.w * centerx) + (float)frame->spriteSourceSize.x + (float)frame->spriteSourceSize.w * 0.5f) * xscale;
            float fy = (((float)-frame->sourceSize.h * centery) + (float)frame->spriteSourceSize.y + (float)frame->spriteSourceSize.h * 0.5f) * yscale;
            float radian = (float)M_PI * degrees / 180.0f;
            float sf = sinf(radian);
            float cf = cosf(radian);
            playdate->graphics->drawRotatedBitmap(bitmap->bitmap, x + (int)(fx * cf - fy * sf), y + (int)(fx * sf + fy * cf), degrees, 0.5f, 0.5f, xscale, yscale);
        }
    }
}

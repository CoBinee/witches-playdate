...
static void CatStay(struct Cat *cat)
{
    ...
    if (cat->actor.state == 0) {
        ...
        // ミリ秒の設定
        cat->millisecond = (rand() % 7000) + 3000;
...
static void CatWalk(struct Cat *cat)
{
    ...
    if (cat->actor.state == 0) {
        ...
        // 向きの設定
        cat->direction = (float)((rand() & 0x02) - 1);

        // ミリ秒の設定
        cat->millisecond = (rand() % 8000) + 1500;
    ...
    if (cat->position.x <= (float)kCatRangeLeft || cat->position.x >= (float)kCatRangeRight) {
